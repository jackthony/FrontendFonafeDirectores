...
import { CONFIG_ACTIVE_DIALOG_TYPE_DIRECTOR, CONFIG_INACTIVE_DIALOG_TYPE_DIRECTOR, MAINTENANCE_TYPE_DIRECTOR_HEADER_TABLE } from 'app/modules/system-maintenance/config/maintenance-type-director.config';
...
export default class MaintenanceTypeDirectorComponent {
	...
	searchTable(resetIndexTable?: boolean): void {
		...
		this._sectorService.getByPagination(this.paramSearchTable(), this.pageIndexTable(), PAGINATOR_PAGE_SIZE, this.filterState()).pipe(
			...
			next: ((response: ResponseEntity<TypeDirectorEntity>) => {
			...
			this._sectorService
				...
				.subscribe({
					next: (response: ResponseEntity<boolean>) => {
						if (response.isSuccess) {
							const messageToast = data.bActivo ? 'Tipo de director eliminado exitosamente' : 'Tipo de director activado exitosamente'; // Muestra un mensaje de éxito
