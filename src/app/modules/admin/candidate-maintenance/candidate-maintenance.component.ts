...
export class CandidateMaintenanceComponent {
  ...
  dataTableActivities = signal<SegUser[]>([]); // Datos de la tabla relacionados con los usuarios
  iconsTable = signal<IconOption<SegUser>[]>([]); // Iconos personalizados para la tabla de usuarios
  lstTypedocument = input<Constant[]>([]); // Lista de tipos de documentos
  
  ministries = signal<Ministry[]>([]); // Lista de ministerios disponibles
