...
export class DialogMaintenanceRoleFormComponent {
	...
	public data: { object: Role } = inject(MAT_DIALOG_DATA);
    ...
    registerForm(): void {
        ...
        this._roleService
            .create(request) // Llama al servicio MinistryService para registrar el rol
    ...
    updateForm(): void {
        ...
        request.request = this.form.value; // Crea una solicitud con los valores del formulario
        this._roleService
            .update(request) // Llama al servicio MinistryService para actualizar el rol
            .pipe(finalize(() => this.loadingService.set(false))) // Finaliza la operación y desactiva el estado de carga
            .subscribe({
                next: (response: ResponseModel<Role>) => {
                    if (response.isSuccess) this.dialogRef.close(true); // Cierra el diálogo si la operación es exitosa
